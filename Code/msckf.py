...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        for msg in self.imu_msg_buffer:
            ...
            ang_vel.append(np.array(msg.angular_velocity))
            lin_acc.append(np.array(msg.linear_acceleration))

        ang_vel = np.array(ang_vel)
        lin_acc = np.array(lin_acc)

        ang_mean = np.mean(ang_vel,axis = 0)
        lin_mean = np.mean(lin_acc,axis = 0)
        
        gyro_bias = ang_mean.flatten()
        self.state_server.imu_state.gyro_bias = gyro_bias

        # Find the gravity in the IMU frame.
        ...
        gravity_imu = lin_mean.flatten()
        ...
        ...
        quat_to_world = from_two_vectors(-IMUState.gravity,gravity_imu)

        self.state_server.imu_state.orientation = quat_to_world
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        R_w_i = to_rotation(imu_state.orientation).T

        F[:3, :3] = -skew(gyro)
        F[:3, 3:6] = -np.identity(3)
        F[6:9, :3] = -R_w_i @ skew(acc)
        F[6:9, 9:12] = -R_w_i
        ...
        ...
        R_k1 = to_rotation(imu_state.orientation_null)
        Phi[:3, :3] = to_rotation(imu_state.orientation) @ R_k1.T

        u = R_k1 @ IMUState.gravity
        s = u / (u @ u)
        ...
        ...
        self.state_server.state_cov = (self.state_server.state_cov + self.state_server.state_cov.T) * 0.5    
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        ...
        if norm_gyro > 1e-5:
            dq_dt = (np.cos(norm_gyro*dt*0.5) * np.identity(4) + np.sin(norm_gyro*dt*0.5)/norm_gyro * omega) @ orientation
            dq_dt2 = (np.cos(norm_gyro*dt*0.25) * np.identity(4) + np.sin(norm_gyro*dt*0.25)/norm_gyro * omega) @ orientation
       
        else:
            dq_dt = np.cos(norm_gyro*dt*0.5) * (np.identity(4) + omega*dt*0.5) @ orientation
            dq_dt2 = np.cos(norm_gyro*dt*0.25) * (np.identity(4) + omega*dt*0.25) @ orientation

        dR_dt_trans = to_rotation(dq_dt).T
        dR_dt2_trans = to_rotation(dq_dt2).T
        
        # Apply 4th order Runge-Kutta 
        # k1 = f(tn, yn)
        ...
        k1_pd = vel
        k1_vd = to_rotation(orientation).T @ acc + IMUState.gravity

        # k2 = f(tn+dt/2, yn+k1*dt/2)
        ...
        k1_v = vel + k1_vd*dt*0.5
        k2_pd = k1_v
        k2_vd = dR_dt2_trans @ acc + IMUState.gravity
        
        # k3 = f(tn+dt/2, yn+k2*dt/2)
        ...
        k2_v = vel + k2_vd*dt*0.5
        k3_pd = k2_v
        k3_vd = dR_dt2_trans @ acc + IMUState.gravity
        
        # k4 = f(tn+dt, yn+k3*dt)
        ...
        k3_v = vel + k3_vd*dt*0.5
        k4_pd = k3_v
        k4_vd = dR_dt_trans @ acc + IMUState.gravity

        # yn+1 = yn + dt/6*(k1+2*k2+2*k3+k4)
        orientation = dq_dt / np.linalg.norm(dq_dt)
        vel = vel + (k1_vd + 2*k2_vd + 2*k3_vd + k4_vd)*dt/6.
        pos = pos + (k1_pd + 2*k2_pd + 2*k3_pd + k4_pd)*dt/6.
    ...
    def state_augmentation(self, time):
        ...
        imu_state = self.state_server.imu_state
        R_ic = imu_state.R_imu_cam0
        t_ci = imu_state.t_cam0_imu


        # Add a new camera state to the state server.
        ...
        ''' base frame = first, target frame = second '''
        R_wi = to_rotation(imu_state.orientation)
        R_wc = R_ic @ R_wi
        t_cw = imu_state.position + R_wi.T @ t_ci

        cam_state = CAMState(imu_state.id)

        cam_state.timestamp = time
        cam_state.orientation = to_quaternion(R_wc)
        cam_state.position = t_cw
    ...
    def add_feature_observations(self, feature_msg):
        ...
        state_id = self.state_server.imu_state.id
        curr_feat_num = len(self.map_server)
        tracked_feat_num = 0
        ...
        ...
        self.tracking_rate = tracked_feat_num / (curr_feat_num+1e-5)
    ...
    def measurement_update(self, H, r):
        ...
        for i, (id,cam_state) in enumerate(self.state_server.cam_states.items()):
            ...
            del_x_cam = delta_x[21+i*6:27+i*6]
            dq_cam = small_angle_quaternion(del_x_cam[:3])
            cam_state.orientation = quaternion_multiplication(dq_cam,cam_state.orientation)

            cam_state.position += del_x_cam[3:]

        # Update state covariance.
        ...
        I_K = np.identity(len(K))-(K @ H_thin)  # Eq. 31 in MSCKF paper
        state_cov = I_K @ self.state_server.state_cov

        # Fix the covariance to be symmetric
        ...
        self.state_server.state_cov=(state_cov +state_cov.T)*0.5
